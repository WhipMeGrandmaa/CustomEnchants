...
import static org.mineacademy.fo.remain.CompMaterial.*;

@AutoRegister
public final class ArmorListener implements Listener {

	private final List<CompMaterial> blockedMaterials = Arrays.asList(
			FURNACE,
			BLAST_FURNACE,
			FURNACE_MINECART,
			CHEST,
		...
		if (e.getAction() == Action.RIGHT_CLICK_AIR || e.getAction() == Action.RIGHT_CLICK_BLOCK) {
			...
			if (!e.useInteractedBlock().equals(Result.DENY)) {
				if (e.getClickedBlock() != null && e.getAction() == Action.RIGHT_CLICK_BLOCK && !player.isSneaking()) {// Having both of these checks is useless, might as well do it though.
					...
					CompMaterial mat = CompMaterial.fromMaterial(e.getClickedBlock().getType());
					for (CompMaterial s : blockedMaterials) {
						if (mat == s) return;
					}
				}
			}
